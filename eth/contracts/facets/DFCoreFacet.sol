...
contract DFCoreFacet is WithStorage {
    ...
    event QuasarSilverLoot(uint256 loc, address oldOwner, address newOwner, uint256 amount);
    ...
    function withdrawSilver(uint256 locationId, uint256 amount) public notPaused disabled {
    ...
    function setQuasarLootSilver(
        ...
        if (gs().revealedCoords[locationId].locationId == 0) {
            revealLocation(_a, _b, _c, _input);
        }

        refreshPlanet(locationId);
        address oldOwner = gs().planets[locationId].owner;
        LibPlanet.setQusarLootSilver(locationId, amount);
        emit QuasarSilverLoot(locationId, oldOwner, msg.sender, amount);
        ls().setQuasarLootSilverCnt++;
        ls().playerLog[msg.sender].setQuasarLootSilverCnt++;
    }
    ...
    function deactivateArtifact(uint256 locationId) public notPaused disabled {
        LibPlanet.refreshPlanet(locationId);
    ...
    function prospectPlanet(uint256 locationId) public notPaused disabled {
        LibPlanet.refreshPlanet(locationId);
    ...
    ) public notPaused notTokenEnded disabled {
    ...
    function depositArtifact(uint256 locationId, uint256 artifactId) public notPaused disabled {
    ...
    function withdrawArtifact(uint256 locationId, uint256 artifactId) public notPaused disabled {
        LibPlanet.refreshPlanet(locationId);
     ...
     */
    function giveSpaceShips(uint256 locationId) public onlyWhitelisted disabled {
