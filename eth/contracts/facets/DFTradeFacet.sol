...
contract DFTradeFacet is WithStorage {
    ...
    ) public payable onlyWhitelisted notPaused disabled {
    ...
    // Function to buy energy for a planet, available only to the owner
        ...
        notPaused
    {
        ...
        require(
            block.timestamp - gs().players[msg.sender].lastBuyEnergyTimestamp >
                gameConstants().BUY_ENERGY_COOLDOWN,
            "wait for cooldown before buying energy again"
        );

        uint256 baseFeePreSecond = 0.00001 ether;
        uint256 fee = baseFeePreSecond * duration;
        if (gs().halfPrice) fee = fee / 2;
        require(msg.value == fee, "Wrong value sent");
    ...
    function buySpaceship(uint256 locationId, ArtifactType artifactType)
        public
        payable
        notPaused
        disabled
    ...
    function donate(uint256 amount) public payable disabled {
    ...
    function buySkin(uint256 _location, uint256 hatType) public payable notPaused disabled {
        require(gs().planets[_location].isInitialized == true, "Planet is not initialized");
        LibPlanet.refreshPlanet(_location);
