...
contract DFGetterTwoFacet is WithStorage {
    ...
    function getLog() public view returns (uint256[] memory ret) {
        ...
        ret[9] = ls().setQuasarLootSilverCnt;
        ret[10] = ls().claimLocationCnt;
        ret[11] = ls().changeArtifactImageTypeCnt;
        ret[12] = ls().deactivateArtifactCnt;
        ret[13] = ls().prospectPlanetCnt;
        ret[14] = ls().findArtifactCnt;
        ret[15] = ls().depositArtifactCnt;
        ret[16] = ls().withdrawArtifactCnt;
        ret[17] = ls().giveSpaceShipsCnt;
        ret[18] = ls().kardashevCnt;
        ret[19] = ls().blueLocationCnt;
        ret[20] = ls().createLobbyCnt;
        ret[21] = ls().moveCnt;
        ret[22] = ls().burnLocationCnt;
        ret[23] = ls().pinkLocationCnt;
        ret[24] = ls().buyPlanetCnt;
        ret[25] = ls().buyPlanetEarn;
        ret[26] = ls().buySpaceshipCnt;
        ret[27] = ls().buySpaceshipCost;
        ret[28] = ls().donateCnt;
        ret[29] = ls().donateSum;
        ret[30] = ls().buyEnergyCnt;
        ret[31] = ls().buyEnergyEarn;
    }
    ...
    function bulkGetPlayerHatSpent(address player, uint256[] memory hatTypes)
        public
        view
        returns (uint256[] memory ret)
    {
    ...
    function getHatPlayerAccounts(uint256 hatType) public view returns (address[] memory) {
        // ret = new address[](ls().hatPlayerAccounts[hatType].length);
        return ls().hatPlayerAccounts[hatType];
    }

    function getHatPlayerSpent(uint256 hatType, address player) public view returns (uint256) {
        return ls().hatPlayerSpent[hatType][player];
    }

    function bulkGetHatPlayerSpent(uint256 hatType, address[] memory players)
        public
        view
        returns (uint256[] memory ret)
    {
