...
async function getPlayerLog(
  ...
  const setQuasarLootSilverCnt = rawPlayerLog.setQuasarLootSilverCnt.toNumber();
  ...
  console.log('    setQuasarLootSilver Cnt:', setQuasarLootSilverCnt);
...
async function analysisGameLog({}, hre: HardhatRuntimeEnvironment) {
  ...
  const setQuasarLootSilverCnt = rawResult[9].toNumber();
  const claimLocationCnt = rawResult[10].toNumber();
  const changeArtifactImageTypeCnt = rawResult[11].toNumber();
  const deactivateArtifactCnt = rawResult[12].toNumber();
  const prospectPlanetCnt = rawResult[13].toNumber();
  const findArtifactCnt = rawResult[14].toNumber();
  const depositArtifactCnt = rawResult[15].toNumber();
  const withdrawArtifactCnt = rawResult[16].toNumber();
  const giveSpaceShipsCnt = rawResult[17].toNumber();
  const kardashevCnt = rawResult[18].toNumber();
  const blueLocationCnt = rawResult[19].toNumber();
  const createLobbyCnt = rawResult[20].toNumber();
  const moveCnt = rawResult[21].toNumber();
  const burnLocationCnt = rawResult[22].toNumber();
  const pinkLocationCnt = rawResult[23].toNumber();
  const buyPlanetCnt = rawResult[24].toNumber();
  const buyPlanetEarn = hre.ethers.utils.formatUnits(rawResult[25]);
  const buySpaceshipCnt = rawResult[26].toNumber();
  const buySpaceshipCost = hre.ethers.utils.formatUnits(rawResult[27]);
  const donateCnt = rawResult[28].toNumber();
  const donateSum = hre.ethers.utils.formatUnits(rawResult[29]);
  const buyEnergyCnt = rawResult[30].toNumber();
  const buyEnergyEarn = hre.ethers.utils.formatUnits(rawResult[31]);
  ...
  console.log('           pinkLocation Cnt:', pinkLocationCnt);
  console.log('              buyPlanet Cnt:', buyPlanetCnt);
  console.log('               buEnergy Cnt:', buyEnergyCnt);
  console.log('           buySpaceship Cnt:', buySpaceshipCnt);
  console.log('                 donate Cnt:', donateCnt);
}
