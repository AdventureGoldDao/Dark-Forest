...
export async function deployAndCut(
  ...
  const getterThreeFacet = await deployGetterThreeFacet({}, libraries, hre);
  ...
  const darkForestFacetCuts = [
    ...
    ...changes.getFacetCuts('DFGetterThreeFacet', getterThreeFacet),
...
export async function deployGetterThreeFacet(
  { },
  { LibGameUtils }: Libraries,
  hre: HardhatRuntimeEnvironment
) {
  const factory = await hre.ethers.getContractFactory('DFGetterThreeFacet', {
    libraries: {
      // LibGameUtils,
    },
  });
  ...
  console.log('------ tx:', contract.address, ' ------');
  await contract.deployTransaction.wait();
  console.log('DFGetterThreeFacet deployed to:', contract.address);
  return contract;
}
