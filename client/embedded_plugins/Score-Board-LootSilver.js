function print_score(players) {
  ...
  for (let player of players) {
    console.log(`${player.player}: ${player.lootSilver}`);
  }
}
...
class Plugin {
  constructor() {
    ...
    this.end_time = new Date('2025-08-17T14:00:00.000Z');
  ...
  update_score = (players) => {
    ...
    for (let player of players) {
      ...
      // let score = df.getPlayerScore(address);

      let score = Math.floor(player.lootSilver / 1000);
      this.scoreboard.set(player.player.toLowerCase(), score);
    }
