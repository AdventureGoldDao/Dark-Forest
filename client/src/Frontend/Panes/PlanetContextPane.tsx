...
import { OpenSetLootSilverButton } from '../Components/OpenSetLootSilverButton';
...
function PlanetContextPaneContent({
  ...
  let lootSilverRow = null;

  if (!p?.destroyed && !p?.frozen && p?.planetType === PlanetType.SILVER_BANK && owned) {
    lootSilverRow = (
      <OpenSetLootSilverButton
        modal={modal}
        planetId={p?.locationId}
        key={PlanetPaneName.LootSilver}
      />
    );
  }
  ...
  if (broadcastRow) rows.push(broadcastRow);
  ...
  if (lootSilverRow) rows.push(lootSilverRow);
