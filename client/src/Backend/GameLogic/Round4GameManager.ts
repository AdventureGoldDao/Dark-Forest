import { CONTRACT_PRECISION, EMPTY_ADDRESS } from '@dfares/constants';
...
import {
  ...
  isUnconfirmedSetQuasarLootSilverTx,
...
import {
  ...
  UnconfirmedSetQuasarLootSilver,
...
class Round4GameManager extends BaseGameManager {
  ...
  public async setQuasarLootSilver(
    locationId: LocationId,
    amount: number
  ): Promise<Transaction<UnconfirmedSetQuasarLootSilver>> {
    try {
      if (!this.account) {
      ...
      if (planet.transactions?.hasTransaction(isUnconfirmedSetQuasarLootSilverTx)) {
        throw new Error('another action is already in progress for this planet');
      }

      if (amount > planet.silver + planet.lootSilver) {
        throw new Error('not enough silver');
      }

      if (planet.destroyed || planet.frozen) {
        throw new Error("can't operate on destoryed or frozen planet");
      }

      localStorage.setItem(
        `${this.getAccount()?.toLowerCase()}-setQuasarLootSilver-locationId`,
        locationId
      );
      ...
      const getArgs = async () => {
        ...
        return [...revealArgs, amount * CONTRACT_PRECISION];
      };

      const txIntent: UnconfirmedSetQuasarLootSilver = {
        methodName: 'setQuasarLootSilver',
        contract: this.contractsAPI.contract,
      ...
      this.getNotificationsManager().txInitError('setQuasarLootSilver', e.message);

      throw e;
    }
  }

  public timeUtilNextBuyEnergyAvailable() {
    if (!this.account) {
      throw new Error('no account set');
    }
  ...
  public getNextBuyEnergyAvailableTimestamp() {
    return Date.now() + this.timeUtilNextBuyEnergyAvailable();
  }
  ...
  public async buyEnergy(
    ...
    try {
      ...
      let energyCostEth = halfPrice ? 0.5 * duration * 0.00001 : duration * 0.00001;

      energyCostEth = Math.round(energyCostEth * 1000000) / 1000000;
      ...
      let fee = bigInt(1e13)
        .multiply(Math.ceil(energyCostEth * 100000))
        .toString();
