...
import { createContract, EthConnection, verifySignature, weiToEth } from '@dfares/network';
...
class BaseGameManager extends EventEmitter {
  /**
   * This variable contains the internal state of objects that live in the game world.
   */
  protected readonly entityStore: GameObjects;
   ...
   * bar of the game.
   */
  protected readonly terminal: React.MutableRefObject<TerminalHandle | undefined>;
   ...
   */
  protected readonly account: EthAddress | undefined;
   ...
   */
  protected readonly contractsAPI: ContractsAPI;
   ...
   */
  protected readonly persistentChunkStore: PersistentChunkStore;

  /**
   * Responsible for generating snark proofs.
   */
  protected readonly snarkHelper: SnarkArgsHelper;
   ...
   */
  protected readonly useMockHash: boolean;
   ...
   * @todo move this into a separate `GameConfiguration` class.
   */
  protected readonly contractConstants: ContractConstants;
   ...
   */
  protected readonly endTimeSeconds: number = new Date('2025-08-17T14:00:00.000Z').getTime() / 1000;
   ...
   */
  protected readonly ethConnection: EthConnection;
   ...
   */
  protected readonly hashConfig: HashConfig;
   ...
   */
  protected readonly planetHashMimc: (...inputs: number[]) => BigInteger;
   ...
   * @todo move this into a new `PlayerState` class.
   */
  protected homeLocation: WorldLocation | undefined;
   ...
   */
  protected captureZoneGenerator: CaptureZoneGenerator | undefined;
  ...
  protected async uploadDiagnostics() {
    eventLogger.logEvent(EventType.Diagnostics, this.diagnostics);
  }

  protected async refreshNetworkHealth() {
    try {
  ...
  protected async hardRefreshPlayer(address?: EthAddress): Promise<void> {
    if (!address) return;
  ...
  protected async hardRefreshPlayerSpaceships(address?: EthAddress, show?: boolean): Promise<void> {
    if (!address) return;
  ...
  protected async getArrival(arrivalId: number): Promise<void> {
    const arrival = await this.contractsAPI.getArrival(arrivalId);
    console.log(arrival);
  }

  protected async getArrivalsForPlanet(planetId: LocationId): Promise<void> {
  ...
  protected async softRefreshPlanet(planetId: LocationId): Promise<void> {
  ...
  protected async bulkHardRefreshPlanets(planetIds: LocationId[]): Promise<void> {
  ...
  protected onTxSubmit(tx: Transaction): void {
  ...
  protected onTxConfirmed(tx: Transaction) {
  ...
  protected onTxReverted(tx: Transaction) {
  ...
  protected onTxCancelled(tx: Transaction) {
  ...
  protected initMiningManager(homeCoords: WorldCoords, cores?: number): void {
  ...
  protected setRadius(worldRadius: number) {
    this.worldRadius = worldRadius;
  ...
  protected async refreshTwitters(): Promise<void> {
    const addressTwitters = await getAllTwitters();
    this.setPlayerTwitters(addressTwitters);
  }

  protected setPlayerTwitters(twitters: AddressTwitterMap): void {
   ...
   */
  protected locationFromCoords(coords: WorldCoords): WorldLocation {
    return {
      coords,
  ...
  protected async findRandomHomePlanet(_selectedCoords: {
    x: number;
    y: number;
   ...
   */
  protected async submitPlanetMessage(
    locationId: LocationId,
   ...
   */
  protected async verifyMessage(message: SignedMessage<unknown>): Promise<boolean> {
  ...
  public async buySpaceship(planetId: LocationId): Promise<Transaction<UnconfirmedBuySpaceship>> {
    try {
      ...
      localStorage.setItem(`${this.getAccount()?.toLowerCase()}-halfPrice`, fee.toString());
   ...
   */
  addNewChunk(chunk: Chunk): BaseGameManager {
